...
`, cmd)
	...
	if (r.datachannel || r.qlog || r.roqMapping != 0) && !(r.roqServer || r.roqClient) {
		fmt.Fprintf(os.Stderr, "Flag -%v, -%v, -%v and only valid for RoQ\n", "dc", r.qlog, "roq-mapping")
		fs.Usage()
		os.Exit(1)
	}
...
func (r *Receive) setupUDP() error {
	...
	rtcpSink, err := gstreamer.NewUDPSink(r.remoteAddr, uint32(r.rtcpSendPort))
	if err != nil {
		return err
	}
	...
	rtcpSrc, err := gstreamer.NewUDPSrc(r.localAddr, uint32(r.rtcpRecvPort))
	if err != nil {
		return err
	}
